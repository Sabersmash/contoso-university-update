...
namespace ContosoUniversity.Api
{
    public class Startup
    {
        ...
        {
            services.AddCustomizedContext(Configuration, CurrentEnvironment).AddAutoMapper(cfg => { cfg.AddProfile<ApiProfile>(); }).AddCustomizedMvc(CurrentEnvironment)
                .AddSwaggerGen(c => { c.SwaggerDoc("v1", new OpenApiInfo { Title = "Contoso University Api", Version = "v1" }); });
        ...
        public void Configure(IApplicationBuilder app, ILoggerFactory loggerFactory, IDbInitializer dbInitializer)
        {
            ...
            // }
            app.UseAuthentication().UseDefaultFiles().UseStaticFiles().UseSwagger().UseSwaggerUI(c => { c.SwaggerEndpoint("/swagger/v1/swagger.json", "Contoso API V1"); });
